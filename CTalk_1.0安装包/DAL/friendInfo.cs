...
namespace CSLG.CTalk.DAL
{
	...
	public partial class friendInfo
	{
		...
		public int GetMaxId()
		{
		return DbHelperSQL.GetMaxID("ID", "friendInfo"); 
		}
		...
		public bool Exists(int ID)
		{
			...
			strSql.Append("select count(1) from friendInfo");
		...
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("insert into friendInfo(");
			strSql.Append("userID,friendID,friendName)");
		...
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("update friendInfo set ");
		...
		public bool Delete(int ID)
		{
			...
			StringBuilder strSql=new StringBuilder();
			strSql.Append("delete from friendInfo ");
			strSql.Append(" where ID=@ID");
			SqlParameter[] parameters = {
					new SqlParameter("@ID", SqlDbType.Int,4)
};
		...
		public CSLG.CTalk.Model.friendInfo GetModel(int ID)
		{
			...
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select  top 1 ID,userID,friendID,friendName from friendInfo ");
			strSql.Append(" where ID=@ID");
			SqlParameter[] parameters = {
					new SqlParameter("@ID", SqlDbType.Int,4)
};
			...
			CSLG.CTalk.Model.friendInfo model=new CSLG.CTalk.Model.friendInfo();
			...
			if(ds.Tables[0].Rows.Count>0)
			{
				...
				if(ds.Tables[0].Rows[0]["friendID"]!=null && ds.Tables[0].Rows[0]["friendID"].ToString()!="")
				{
		...
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select ID,userID,friendID,friendName ");
			strSql.Append(" FROM friendInfo ");
			if(strWhere.Trim()!="")
			{
		...
		public DataSet GetList(int Top,string strWhere,string filedOrder)
		{
			...
			strSql.Append(" ID,userID,friendID,friendName ");
			strSql.Append(" FROM friendInfo ");
			if(strWhere.Trim()!="")
			{
				strSql.Append(" where "+strWhere);
